...
class Remessa
{
    ...
    private function modelo2(Bill $bill)
    {
        ...
        $remessa = new \Eduardokum\LaravelBoleto\Cnab\Remessa\Cnab400\Banco\Caixa(
            [
                'agencia' => $company->getAgencia(),
                'idRemessa' => $bill->getBillRemessa()->getId(),
        ...
        echo $remessa->save($this->remessasPath . '/remessa_m1_' . $bill->getBillRemessa()->getId() . '.txt');
    }
    ...
    private function modelo1(Bill $bill)
    {
        ...
        $arquivo->configure([
            ...
            'numero_sequencial_arquivo' => $bill->getBillRemessa()->getId()
        ]);
        ...
        $arquivo->save($this->remessasPath . '/remessa_m2_' . $bill->getBillRemessa()->getId() . '.txt');
    }

    private function modelo3(Bill $bill)
    {
        $company = new Company();

        $arquivo = new \CnabPHP\Remessa(104, 'cnab240_SIGCB', array(
            ...
            'numero_sequencial_arquivo' => $bill->getBillRemessa()->getId(), // sequencial do arquivo um numero novo para cada arquivo gerado
        ));
        ...
        $filename = $this->remessasPath . '/remessa_m3_' . $bill->getBillRemessa()->getId() . '.txt';
        file_put_contents($filename, $text);
    }

    public function download(Bill $bill)
    {
        $file = $this->remessasPath . '/remessa_' . $bill->getBillRemessa()->getId() . '.pdf';
        ...
        if (file_exists($file)) {
            return new Response(
                ...
                [
                    ...
                    //'Content-Disposition' => 'attachment; filename="fatura_' . $bill->getBillRemessa()->getId() . '.pdf"'
                    'Content-Disposition' => 'inline; filename="remessa_' . $bill->getBillRemessa()->getId() . '.txt"'
                ]
            );
        }
        return false;
